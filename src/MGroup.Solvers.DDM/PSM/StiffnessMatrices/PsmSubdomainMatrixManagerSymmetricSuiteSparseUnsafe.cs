...
{
	public class PsmSubdomainMatrixManagerSymmetricSuiteSparseUnsafe : IPsmSubdomainMatrixManager
	{
		...
		private CsrMatrixFaster Kbb;
		private CsrMatrixFaster Kbi;
		private SymmetricCscMatrix Kii;
		private CholeskySuiteSparse inverseKii;

		public PsmSubdomainMatrixManagerSymmetricSuiteSparseUnsafe(
		...
		public IMatrixView CalcSchurComplement()
		{
			...
			TriangularUpper kbbUpper = Kbb.AsSimpleCsr.CopyToUpperPacked();
			var kbbSymm = SymmetricMatrix.CreateFromPackedColumnMajorArray(kbbUpper.RawData);
			return SchurComplementPckCsrCscSym.CalcSchurComplement(kbbSymm, Kbi.AsSimpleCsr, inverseKii);
		}
		...
		public void ExtractKiiKbbKib()
		{
			...
			Kbb = new CsrMatrixFaster(submatrixExtractor.Submatrix00);
			Kbi = new CsrMatrixFaster(submatrixExtractor.Submatrix01);
		...
		public class Factory : IPsmSubdomainMatrixManagerFactory<SymmetricCscMatrix>
		{
			...
			public IPsmSubdomainMatrixManager CreateMatrixManager(
				SubdomainLinearSystem<SymmetricCscMatrix> linearSystem, PsmSubdomainDofs subdomainDofs)
				=> new PsmSubdomainMatrixManagerSymmetricSuiteSparseUnsafe(linearSystem, subdomainDofs);
		}
	}
}
